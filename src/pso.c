...
void pso_set_default_settings(pso_settings_t *settings) {
	...
	*/
	...
	settings->rng = NULL;
	settings->seed = time(0); //initialise random seed
...
{
	int free_rng = 0; //free the rng when finished with it
	// Particles
    	...
    	// Swarm
    	double pos_nb[settings->size][settings->dim]; // what is best informed
